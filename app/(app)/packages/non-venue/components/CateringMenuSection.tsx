"use client";

import { motion, AnimatePresence } from "framer-motion";
import { useState } from "react";

const packageData = {
  dahlia: [
  {
    title: "Dekorasi Pelaminan",
    items: [
      "Dekorasi 8m mix fresh flowers",
      "5 spot light",
      "1 set kursi pelaminan",
      "Taman depan pelaminan 7m",
    ],
  },

  {
    title: "Dekorasi Penerima Tamu",
    items: [
      "2 meja penerima tamu",
      "2 dekorasi meja tamu (simple flower)",
      "2 kotak angpau",
      "1 welcome gate penerima tamu",
    ],
  },

  {
    title: "Dekorasi Gedung",
    items: [
      "1 set dekorasi meja kursi akad nikah",
      "Dekorasi lorong jalan 10m",
      "1 welcome sign mirror",
      "6 standing flower",
      "1 handbouquet",
      "1 backdrop photobooth/selfie corner",
    ],
  },

  {
    title: "MUA & Attire",
    items: [
      "Makeup akad / pemberkatan",
      "Retouch resepsi",
      "1 pasang busana akad",
      "1 pasang busana resepsi",
      "Hijab do / hair do",
      "Accessories akad & resepsi nasional",
      "2 makeup dan busana ibu",
      "2 busana bapak",
      "Hijab do ibu",
    ],
  },

  {
    title: "Dokumentasi",
    items: [
      "1 photographer & 1 videographer",
      "Teaser 1 menit & clip 3-4 menit",
      "1 album kolase",
      "Edit photo up to 200",
      "1 pcs flashdisk",
      "Note: rincian item dekorasi akan disesuaikan dengan vendor yang dipilih",
    ],
  },

  {
    title: "Sound & Entertainment",
    items: [
      "Singer",
      "Keyboard",
      "Sound system",
    ],
  },

  {
    title: "MC & WO",
    items: [
      "5 WO on the day",
      "Konsep & rundown acara",
      "Koordinasi dengan keluarga & vendor",
      "1 kali meeting online",
      "1 kali final meeting offline",
      "Teks-teks acara",
      "Property event",
      "Dresscode & HT",
      "7 jam kerja",
    ],
  },

  {
    title: "Wedding Planner",
    items: [
      "Merencanakan acara",
      "Mencari dan membooking vendor",
      "Pendampingan",
      "Free konsultasi 24 jam",
    ],
  },

  {
    title: "Free Bonus",
    items: [
      "Janur",
      "Undangan digital",
      "Henna & Fake nails",
      "Transport all team",
    ],
  },
],

  tulip: [
  {
    title: "Dekorasi Pelaminan",
    items: [
      "Dekorasi 8m mix fresh flowers",
      "5 spot light",
      "1 set kursi pelaminan",
      "Taman depan pelaminan 7m",
    ],
  },

  {
    title: "Dekorasi Penerima Tamu",
    items: [
      "2 meja penerima tamu",
      "2 dekorasi meja tamu (simple flower)",
      "2 kotak angpau",
      "1 welcome gate penerima tamu",
    ],
  },

  {
    title: "Dekorasi Gedung",
    items: [
      "1 set dekorasi meja kursi akad nikah",
      "Dekorasi lorong jalan 10m",
      "1 welcome sign mirror",
      "6 standing flower",
      "1 handbouquet",
      "1 backdrop photobooth/selfie corner",
    ],
  },

  {
    title: "MUA & Attire",
    items: [
      "Makeup akad / pemberkatan",
      "Retouch resepsi",
      "1 pasang busana akad",
      "1 pasang busana resepsi",
      "Hijab do / hair do",
      "Accessories akad & resepsi nasional",
      "2 makeup dan busana ibu",
      "2 busana bapak",
      "Hijab do ibu",
    ],
  },

  {
    title: "Dokumentasi",
    items: [
      "1 photographer & 1 videographer",
      "Teaser 1 menit & clip 3-4 menit",
      "1 album kolase",
      "Edit photo up to 200",
      "1 pcs flashdisk",
      "Note: rincian item dekorasi akan disesuaikan dengan vendor yang dipilih",
    ],
  },

  {
    title: "Catering 300 Pax",
    items: [
      "Nasi putih",
      "Nasi goreng",
      "Aneka masakan ayam",
      "Aneka masakan daging",
      "Aneka sop",
      "Sayuran",
      "Masakan pendamping",
      "Dessert puding & kue",
      "Soft drink",
      "Buah potong",
      "Air mineral",
      "Set meja utama & cover",
      "Waiters",
      "Set alat makan",
      "Trash bag",
      "Test food 1–2 kali",
    ],
  },

  {
    title: "Sound & Entertainment",
    items: [
      "Singer",
      "Keyboard",
      "Sound system",
    ],
  },

  {
    title: "MC & WO",
    items: [
      "5 WO on the day",
      "Konsep & rundown acara",
      "Koordinasi dengan keluarga & vendor",
      "1 kali meeting online",
      "1 kali final meeting offline",
      "Teks-teks acara",
      "Property event",
      "Dresscode & HT",
      "7 jam kerja",
    ],
  },

  {
    title: "Wedding Planner",
    items: [
      "Merencanakan acara",
      "Mencari dan membooking vendor",
      "Pendampingan",
      "Free konsultasi 24 jam",
    ],
  },

  {
    title: "Free Bonus",
    items: [
      "Janur",
      "Undangan digital",
      "Henna & Fake nails",
      "Transport all team",
    ],
  },
],
};

export default function NonVenueDetailSection() {
  const [activePackage, setActivePackage] = useState<"dahlia" | "tulip">(
    "dahlia"
  );

  return (
    <section className="relative py-24 px-6 bg-[#0a0a0a] overflow-hidden">
      
      {/* Background Glow */}
      <div className="absolute top-20 left-20 w-72 h-72 bg-[#d4b15a]/10 blur-[120px] rounded-full" />
      <div className="absolute bottom-20 right-20 w-72 h-72 bg-[#d4b15a]/10 blur-[120px] rounded-full" />

      <div className="relative z-10 max-w-7xl mx-auto">
        
        {/* Heading */}
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          className="text-center mb-14"
        >
          <p className="uppercase tracking-[0.4em] text-[#d4b15a] text-sm mb-4">
            Detail Paket
          </p>

          <h2 className="font-heading text-4xl md:text-6xl text-[#f5e7b2] mb-6">
            Pilih Detail Paket
          </h2>

          <p className="text-neutral-400 max-w-2xl mx-auto leading-8">
            Lihat rincian lengkap dari masing-masing paket wedding
            sesuai kebutuhan acara spesial Anda.
          </p>
        </motion.div>

        {/* Toggle Buttons */}
        <div className="flex justify-center gap-4 mb-14 flex-wrap">
          <button
            onClick={() => setActivePackage("dahlia")}
            className={`px-8 py-3 rounded-full font-semibold transition-all duration-300 ${
              activePackage === "dahlia"
                ? "bg-[#d4b15a] text-black shadow-lg"
                : "bg-white/5 text-white border border-white/10 hover:border-[#d4b15a]/40"
            }`}
          >
            Paket Dahlia
          </button>

          <button
            onClick={() => setActivePackage("tulip")}
            className={`px-8 py-3 rounded-full font-semibold transition-all duration-300 ${
              activePackage === "tulip"
                ? "bg-[#d4b15a] text-black shadow-lg"
                : "bg-white/5 text-white border border-white/10 hover:border-[#d4b15a]/40"
            }`}
          >
            Paket Tulip
          </button>
        </div>

        {/* Package Details */}
        <AnimatePresence mode="wait">
          <motion.div
            key={activePackage}
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -30 }}
            transition={{ duration: 0.5 }}
            className="grid md:grid-cols-2 gap-8"
          >
            {packageData[activePackage].map((section, index) => (
              <motion.div
                key={section.title}
                initial={{ opacity: 0, y: 40 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{
                  delay: index * 0.08,
                }}
                whileHover={{
                  y: -8,
                  scale: 1.02,
                }}
                className="
                  p-8
                  rounded-3xl
                  bg-white/5
                  border border-white/10
                  backdrop-blur-xl
                  hover:border-[#d4b15a]/40
                  transition
                "
              >
                {/* Card Title */}
                <h3 className="font-heading text-2xl md:text-3xl text-[#d4b15a] mb-5">
                  {section.title}
                </h3>

                <div className="w-16 h-[2px] bg-[#d4b15a] mb-6 rounded-full" />

                {/* Items */}
                <ul className="space-y-3">
                  {section.items.map((item, i) => (
                    <li
                      key={i}
                      className="text-neutral-300 text-sm md:text-base flex gap-3 leading-7"
                    >
                      <span className="text-[#d4b15a]">•</span>
                      {item}
                    </li>
                  ))}
                </ul>
              </motion.div>
            ))}
          </motion.div>
        </AnimatePresence>
      </div>
    </section>
  );
}